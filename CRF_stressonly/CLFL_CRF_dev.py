...
# def stressed(syll_label):
#     stressed = "unstressed"
#     if (syll_label == "MORA_HAUPT" or syll_label == "DOPPEL" or syll_label == "HALB_HAUPT" or syll_label == "HALB_NEBEN" or syll_label == "MORA_NEBEN"):
#         stressed = "stressed"
#     else:
#         pass
#
...
def prepcrf_stress(tagged_line_list):

    # create tuples with syllable characteristic features
    training = []
    for line in tagged_line_list:
...
def prepcrf_final(taggedstring):

    # break data into list on line breaks
    tagged = taggedstring.split('\n')
    tagger = pycrfsuite.Tagger()
    tagger.open('MHG_Stress.crfsuite')
    stress_prepped = prepcrf_stress(tagged)
    all_stress_pat = [tagger.tag(l) for l in stress_prepped]
            ...
            for i, tup in enumerate(news):
                if tup[0] == 'WBY':
                    ...
                    if line[i + 1][0] == "WBY":
                        ...
                        wpos = "WBYL"
                    finaltuple = (tup[0], lpos, wpos, tup[1])
                    ltuples.append(finaltuple)
    ...
    training = []
    for i, line in enumerate(ftuples):

        newtups = []
        for t, tup in enumerate(line):
            # adds syllable characteristics to tuple for being feature
            syllweight = (tup[0], syllableend(tup[0]) +
                          syllableweight(tup[0]), tup[1], tup[2], all_stress_pat[i][t], tup[3])
            newtups.append(syllweight)
        training.append(newtups)
    return (training)
...
def syllable2features(line, i):
    ...
    stress = line[i][4]  # stressed or not
    ...
    if i > 0:  # preceding syllable
        ...
        lposn1 = line[i - 1][2]
        wposn1 = line[i - 1][3]
        stress1 = line[i - 1][4]  # stressed or not
        features.extend([
            '-1:i=' + str(i - 1),
...
def line2labels(line):
    return [label for token, syllcharacs, lpos1, wpos1, stress, label in line]


def line2tokens(line):
    return [token for token, syllcharacs, lpos1, wpos1, stress, label in line]
